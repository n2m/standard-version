  ...
  return new Promise((resolve, reject) => {
    if (releaseAs) {
      ...
      conventionalRecommendedBump({
        ...
        whatBump: commits => {
          if (!commits) return {level: 2, reason: 'empty'}
          var level = 2
          var reason = 'patch'
          for (var i = 0; i < commits.length; i++) {
            var c = commits[i]
            if (args.tagPrefix && c.scope && !args.tagPrefix.startsWith(c.scope)) continue

            if (c.notes && c.notes.length > 0) {
              for (var j = 0; j < c.notes.length; j++) {
                var n = c.notes[j]
                if (n.title === 'BREAKING CHANGE') return {level: 0, reason: 'BREAKING CHANGE'}
              }
            }
